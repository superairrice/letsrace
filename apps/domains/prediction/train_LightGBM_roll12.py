...
def train_model_for_period_and_update_exp011(
    ...
    with closing(get_conn()) as conn:
        ...
        df_train["실제순위"] = pd.to_numeric(
            df_train["실제순위"], errors="coerce"
        )
        before_rows = len(df_train)
        df_train = df_train.dropna(subset=["실제순위"]).copy()
        if df_train.empty:
            raise ValueError(
                f"[{from_date} ~ {to_date}] 실제순위가 유효한 학습 데이터가 없습니다."
            )
        dropped = before_rows - len(df_train)
        if dropped:
            print(f"⚠️ 실제순위 NaN {dropped}건 제외")
...
def run_monthly_roll12(run_yyyymm01: Optional[str] = None):
    """
    매월 1일 기준 롤링 12개월 학습 + 해당 월 exp011 업데이트

    run_yyyymm01:
      - 'YYYYMM01' 형식 (예: '20251201')
      - None 이면 오늘 날짜 기준 이번달 1일로 계산
    """
    ...
    train_from_dt = train_to_dt.replace(year=train_to_dt.year - 1) + timedelta(days=1)
    ...
    update_to = run_date.replace(day=last_day).strftime("%Y%m%d")
...
if __name__ == "__main__":
    # 사용법:
    #   python train_LightGBM.py 20251201
    # 인자 없으면: 오늘 날짜 기준으로 '이번달 1일'로 맞춰 실행
    if len(sys.argv) >= 2:
        run_arg = sys.argv[1]
    else:
        today = datetime.now().date()
        run_arg = today.replace(day=1).strftime("%Y%m%d")
