...
def load_result_data_from_db(
    ...
    SELECT
        ...
        e.rno        AS 경주번호,
        e.gate       AS 마번,
        x.grade      AS 등급,       -- ★ 경주등급 추가 (exp010)
        r.distance   AS 경주거리,   -- ★ 경주거리 추가 (rec010)
...
def calc_top6_trifecta_raw(
    ...
    for (track, date, rno), g in df.groupby(["경마장", "경주일", "경주번호"]):
        ...
        grade = g["등급"].iloc[0] if "등급" in g.columns else ""
...
if __name__ == "__main__":
    ...
    out_path = "/Users/Super007/Documents/top6_raw_20241129_20251130_20251214.csv"
