"""지정한 주력/보조 전략만으로 별도 수익 분석을 출력한다."""
...
import 베팅방법별_환수율 as base_mod


PROFILE_NAME = "별도_수익분석_주력보조_무캡"
DEFAULT_FROM_DATE = "20260101"
DEFAULT_TO_DATE = "20261231"
DEFAULT_OUTPUT_PATH = (
    "/Users/Super007/Documents/r_pop_total_new_별도_수익분석_주력보조_무캡.csv"
)
DEFAULT_SATURDAY_WINDOW_OUTPUT_PATH = (
    "/Users/Super007/Documents/r_pop_total_new_별도_수익분석_주력보조_무캡_토요일pm2일.csv"
)
...
def build_admin_profit_analysis_payload_for_range(
    ...
    if race_df is not None and hasattr(race_df, "columns") and not race_df.empty:
        ...
        race_df = base_mod._augment_anchor1_26_trifecta(race_df, bet_unit=100)
        race_df = base_mod._augment_anchor2_37_trifecta(race_df, bet_unit=100)
        race_df = base_mod._augment_top4pair_56_trio(race_df, bet_unit=100)
        if race_df.empty:
            race_df = None
    ...
    if race_df is not None and hasattr(race_df, "columns") and not race_df.empty:
        ...
        for track_name in ["서울", "부산"]:
            ...
            for group_label, strategy_keys in admin_profit_groups.get(
                ...
                detail_methods.sort(key=base_mod._method_display_sort_key)
...
def main() -> None:
    ...
    base_mod.PROFILE_NAME = PROFILE_NAME
    base_mod.print_payload(payload)
    base_mod.save_payload_df(payload, DEFAULT_OUTPUT_PATH)
    ...
    print("\n[토요일 ±2일 기준 집계]")
    base_mod.print_payload(saturday_pm2_payload)
    base_mod.save_payload_df(
