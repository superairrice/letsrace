...
def _build_combined_track_total(payload: dict) -> dict:
    ...
    total_bet = float(sum(float(track.get("total_bet", 0.0) or 0.0) for track in tracks))
    total_refund = float(sum(float(track.get("total_refund", 0.0) or 0.0) for track in tracks))
    ...
    for track in payload.get("method_bet_by_track") or []:
        for method in track.get("methods") or []:
            ...
            if method.get("holes_per_race") is not None:
                item["holes_per_race_values"].add(int(method.get("holes_per_race", 0) or 0))
...
def flatten_payload_to_df(payload: dict) -> pd.DataFrame:
    ...
    summary_hit_rate = _calc_hit_rate(summary_total.get("hits", 0), summary_total.get("races", 0))
    rows.append(
        {
        ...
        rows.append(
            {
                ...
                "적중율": _calc_hit_rate(track.get("hit_races", 0), track.get("total_races", 0)),
                "적중율_pct": _calc_hit_rate(track.get("hit_races", 0), track.get("total_races", 0))
                * 100.0,
            ...
            rows.append(
                {
                    ...
                    "총환수액": method.get("refund", 0.0),
                    "이익금액": method.get("profit", 0.0),
                    "적중율": _calc_hit_rate(method.get("hits", 0), track.get("total_races", 0)),
                    "적중율_pct": _calc_hit_rate(method.get("hits", 0), track.get("total_races", 0))
                    * 100.0,
                    "환수율": (
                        (float(method.get("refund", 0.0)) / float(method.get("amount", 0.0)))
                        if float(method.get("amount", 0.0) or 0.0) > 0
                        else 0.0
                    ),
                    "환수율_pct": (
                        ((float(method.get("refund", 0.0)) / float(method.get("amount", 0.0))) * 100.0)
                        if float(method.get("amount", 0.0) or 0.0) > 0
                        else 0.0
                    ),
                    "구멍수": method.get("holes_per_race"),
...
def _build_admin_profit_runtime_config():
    global _RUNTIME_CONFIG_CACHE
    if _RUNTIME_CONFIG_CACHE is not None:
        groups, columns, labels = _RUNTIME_CONFIG_CACHE
        return copy.deepcopy(groups), copy.deepcopy(columns), copy.deepcopy(labels)
    ...
    admin_profit_strategy_result_columns = copy.deepcopy(ADMIN_PROFIT_STRATEGY_RESULT_COLUMNS)
    ...
    _RUNTIME_CONFIG_CACHE = (
        copy.deepcopy(admin_profit_groups),
        copy.deepcopy(admin_profit_strategy_result_columns),
        copy.deepcopy(admin_profit_strategy_labels),
    )
    return admin_profit_groups, admin_profit_strategy_result_columns, admin_profit_strategy_labels


def _load_augmented_admin_profit_race_df(from_date: str, to_date: str):
    cache_key = (str(from_date), str(to_date))
    cached_df = _AUGMENTED_RACE_DF_CACHE.get(cache_key)
    ...
    race_df = _apply_max_odds_cap_to_refunds(race_df, bet_unit=100)

    if race_df.empty:
        return None
    _AUGMENTED_RACE_DF_CACHE[cache_key] = race_df.copy()
    return race_df


def _apply_max_odds_cap_to_refunds(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df

    df = race_df.copy()
    trio_odds = pd.to_numeric(df.get("삼복승식배당율"), errors="coerce").clip(upper=MAX_DISPLAY_ODDS)
    trifecta_odds = pd.to_numeric(df.get("삼쌍승식배당율"), errors="coerce").clip(upper=MAX_DISPLAY_ODDS)
    df["삼복승식배당율"] = trio_odds
    df["삼쌍승식배당율"] = trifecta_odds

    _, admin_profit_strategy_result_columns, admin_profit_strategy_labels = _build_admin_profit_runtime_config()
    for key, column_meta in admin_profit_strategy_result_columns.items():
        ...
        if not hit_col or not refund_col or hit_col not in df.columns:
            continue
        ...
        hit_mask = pd.to_numeric(df[hit_col], errors="coerce").fillna(0).gt(0).astype(int)
        refund_values = (odds.fillna(0.0) * bet_unit * hit_mask).astype(float)
        df[refund_col] = refund_values

        derived_refund_col = str(hit_col).replace("적중", "환수액")
        if derived_refund_col in df.columns:
            df[derived_refund_col] = refund_values

    return df


def _augment_top3pair_46_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor2_38_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor2_37_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor2_36_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor2_137_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
    ...
    def _calc_row(row: pd.Series) -> pd.Series:
        ...
        top3_8_12 = _parse_gate_list(row.get("3~8,12_마번"))
        top3_7 = top3_8_12[:5]
        candidate_gates = [anchor1_gate] + top3_7 if anchor1_gate is not None else top3_7
        actual_top3 = _parse_gate_list(row.get("실제_top3_마번"))
        odds = pd.to_numeric(row.get("삼쌍승식배당율"), errors="coerce")
...
def _augment_anchor2_136_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor1_27_trio(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
    ...
    def _calc_row(row: pd.Series) -> pd.Series:
        ...
        top3_8_12 = _parse_gate_list(row.get("3~8,12_마번"))
        top3_7 = top3_8_12[:5]
        candidate_gates = [second_anchor_gate] + top3_7 if second_anchor_gate is not None else top3_7
        actual_top3 = _parse_gate_list(row.get("실제_top3_마번"))
        ...
        refund = float(trio_odds) * bet_unit if hit == 1 and pd.notna(trio_odds) else 0.0
        bet = float(bet_per_race) if valid else 0.0
        return pd.Series(
            {
...
def _augment_anchor1_26_trio(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
    ...
    def _calc_row(row: pd.Series) -> pd.Series:
        ...
        top3_8_12 = _parse_gate_list(row.get("3~8,12_마번"))
        top3_6 = top3_8_12[:4]
        candidate_gates = [second_anchor_gate] + top3_6 if second_anchor_gate is not None else top3_6
        actual_top3 = _parse_gate_list(row.get("실제_top3_마번"))
        trio_odds = pd.to_numeric(row.get("삼복승식배당율"), errors="coerce")
...
def _augment_anchor1_24_trio(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
    ...
    def _calc_row(row: pd.Series) -> pd.Series:
        ...
        top3_8_12 = _parse_gate_list(row.get("3~8,12_마번"))
        top3_4 = top3_8_12[:2]
        candidate_gates = [second_anchor_gate] + top3_4 if second_anchor_gate is not None else top3_4
        actual_top3 = _parse_gate_list(row.get("실제_top3_마번"))
        trio_odds = pd.to_numeric(row.get("삼복승식배당율"), errors="coerce")
...
def _augment_anchor1_25_trio(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
    ...
    def _calc_row(row: pd.Series) -> pd.Series:
        ...
        top3_8_12 = _parse_gate_list(row.get("3~8,12_마번"))
        top3_5 = top3_8_12[:3]
        candidate_gates = [second_anchor_gate] + top3_5 if second_anchor_gate is not None else top3_5
        actual_top3 = _parse_gate_list(row.get("실제_top3_마번"))
        trio_odds = pd.to_numeric(row.get("삼복승식배당율"), errors="coerce")
...
def _augment_anchor12_3_12_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor21_3_12_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
    ...
    def _calc_row(row: pd.Series) -> pd.Series:
        ...
        top3_12_set = set(top3_12)
        valid = (
            pop1_gate is not None
            and pop2_gate is not None
            and len(top3_12) >= 1
        )
        hit = int(
            valid
...
def _augment_anchor21_3_10_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_anchor12_3_10_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_top4pair_56_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_top4pair_57_trifecta(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_top4pair_57_trio(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def _augment_top4pair_56_trio(race_df: pd.DataFrame, bet_unit: int = 100) -> pd.DataFrame:
    if race_df is None or race_df.empty:
        return race_df
...
def build_admin_profit_analysis_payload_for_range(from_date: str, to_date: str) -> dict:
    ...
    if race_df is not None and hasattr(race_df, "columns") and not race_df.empty:
        ...
        if perf_rows:
            ...
            for item in track_stat_map.values():
                ...
                for pop_rank in (1, 2, 3, 4):
                    ...
                    item[f"r_pop{pop_rank}_top3_rate"] = top3_hits / races if races > 0 else 0.0
                    item[f"r_pop{pop_rank}_top1_rate"] = top1_hits / races if races > 0 else 0.0
        ...
        for track_name in ["서울", "부산"]:
            ...
            for group_label, strategy_keys in admin_profit_groups.get(track_name, {}).items():
                ...
                for key in strategy_keys:
                    ...
                    bet_amount = float(track_df[column_meta["bet"]].fillna(0).sum()) if column_meta["bet"] in track_df.columns else 0.0
                    refund_amount = float(track_df[column_meta["refund"]].fillna(0).sum()) if column_meta["refund"] in track_df.columns else 0.0
                    hit_count = int(track_df[column_meta["hit"]].fillna(0).astype(int).sum()) if column_meta["hit"] in track_df.columns else 0
                ...
                group_refund = float(track_df[refund_cols].sum().sum()) if refund_cols else 0.0
                group_profit = group_refund - group_bet
                group_holes_per_race = int(
                    sum(
                        admin_profit_strategy_result_columns.get(key, {}).get("holes_per_race", 0)
                        for key in strategy_keys
                    )
                )
                group_hits = int(track_df[hit_cols].fillna(0).astype(int).gt(0).any(axis=1).sum()) if hit_cols else 0
                methods.append(
                    {
                        "label": group_label,
                        "amount": group_bet,
        ...
        metrics["roi"] = (primary_total_refund / primary_total_bet) if primary_total_bet > 0 else 0.0
...
def _build_saturday_primary_summary(from_date: str, to_date: str) -> list[dict]:
    admin_profit_groups, admin_profit_strategy_result_columns, _labels = _build_admin_profit_runtime_config()
    race_df = _load_augmented_admin_profit_race_df(from_date, to_date)
    if race_df is None or race_df.empty or "경주일" not in race_df.columns:
        return []

    date_series = pd.to_datetime(race_df["경주일"].astype(str), format="%Y%m%d", errors="coerce")
    valid_mask = date_series.notna()
    if not valid_mask.any():
        return []

    def _nearest_saturday(dt: pd.Timestamp):
        weekday = int(dt.weekday())
        prev_sat = dt - pd.Timedelta(days=(weekday - 5) % 7)
        next_sat = prev_sat + pd.Timedelta(days=7)
        if abs((dt - prev_sat).days) <= abs((next_sat - dt).days):
            sat = prev_sat
        else:
            sat = next_sat
        return sat.strftime("%Y%m%d"), abs((dt - sat).days) <= 2

    nearest_info = date_series.apply(lambda dt: _nearest_saturday(dt) if pd.notna(dt) else (None, False))
    race_df = race_df.copy()
    race_df["토요일기준일"] = nearest_info.apply(lambda x: x[0])
    race_df["토요일기준_포함"] = nearest_info.apply(lambda x: bool(x[1]))
    saturday_df = race_df[race_df["토요일기준_포함"]].copy()
    if saturday_df.empty:
        return []

    rows = []
    for sat_date in sorted(saturday_df["토요일기준일"].astype(str).unique()):
        date_df = saturday_df[saturday_df["토요일기준일"].astype(str) == sat_date].copy()
        total_bet = 0.0
        total_refund = 0.0
        hole_sum = 0
        hit_races = 0
        for track_name in ["서울", "부산"]:
            track_date_df = date_df[date_df.get("경마장") == track_name].copy()
            if track_date_df.empty:
                continue
            strategy_keys = (admin_profit_groups.get(track_name, {}) or {}).get("주력베팅", [])
            bet_cols = []
            refund_cols = []
            hit_cols = []
            for key in strategy_keys:
                meta = admin_profit_strategy_result_columns.get(key)
                if not meta:
                    continue
                hole_sum += int(meta.get("holes_per_race", 0) or 0)
                if meta["bet"] in track_date_df.columns:
                    bet_cols.append(meta["bet"])
                if meta["refund"] in track_date_df.columns:
                    refund_cols.append(meta["refund"])
                if meta["hit"] in track_date_df.columns:
                    hit_cols.append(meta["hit"])
            total_bet += float(track_date_df[bet_cols].sum().sum()) if bet_cols else 0.0
            total_refund += float(track_date_df[refund_cols].sum().sum()) if refund_cols else 0.0
            hit_races += (
                int(track_date_df[hit_cols].fillna(0).astype(int).gt(0).any(axis=1).sum())
                if hit_cols
                else 0
            )

        race_count = int(
            date_df[["경마장", "경주일", "경주번호"]].drop_duplicates().shape[0]
            if {"경마장", "경주일", "경주번호"}.issubset(date_df.columns)
            else len(date_df)
        )
        rows.append(
            {
                ...
                "races": race_count,
                "hits": hit_races,
                "hit_rate": _calc_hit_rate(hit_races, race_count),
                "bet": total_bet,
                "refund": total_refund,
                "profit": total_refund - total_bet,
                "roi": (total_refund / total_bet) if total_bet > 0 else 0.0,
                "holes_per_race": hole_sum,
            }
        )
    return rows


def print_saturday_primary_summary(from_date: str, to_date: str) -> None:
    rows = _build_saturday_primary_summary(from_date, to_date)
    if not rows:
        return
    total_races = sum(int(row["races"]) for row in rows)
    total_hits = sum(int(row["hits"]) for row in rows)
    total_bet = sum(float(row["bet"]) for row in rows)
    total_refund = sum(float(row["refund"]) for row in rows)
    holes = int(rows[0].get("holes_per_race", 0) or 0)

    print("[주력베팅 토요일 기준 환수현황]")
    for row in rows:
        print(
            f"[토요일기준 {row['date']}]  경주수: {int(row['races'])}  "
            f"적중경주수: {int(row['hits'])}  적중율: {float(row['hit_rate']):.3f}  "
            f"총베팅액: {float(row['bet']):,.0f}원  총환수액: {float(row['refund']):,.1f}원  "
            f"이익금액: {float(row['profit']):,.1f}원  환수율: {float(row['roi']):.3f}"
        )
    print(
        f"[토요일 총계]  경주수: {total_races}  적중경주수: {total_hits}  "
        f"적중율: {_calc_hit_rate(total_hits, total_races):.3f}  "
        f"총베팅액: {total_bet:,.0f}원  총환수액: {total_refund:,.1f}원  "
        f"이익금액: {total_refund - total_bet:,.1f}원  환수율: {(total_refund / total_bet) if total_bet > 0 else 0.0:.3f}  "
        f"구멍수: {holes}"
    )


def _summarize_strategy_combo(track_df: pd.DataFrame, strategy_keys: list[str], strategy_columns: dict) -> dict:
    bet_cols = []
    refund_cols = []
    hit_cols = []
    holes = 0
    for key in strategy_keys:
        ...
        if not meta:
            continue
        holes += int(meta.get("holes_per_race", 0) or 0)
        if meta["bet"] in track_df.columns:
            bet_cols.append(meta["bet"])
        if meta["refund"] in track_df.columns:
            refund_cols.append(meta["refund"])
        if meta["hit"] in track_df.columns:
            hit_cols.append(meta["hit"])
    ...
    total_bet = float(track_df[bet_cols].sum().sum()) if bet_cols else 0.0
    total_refund = float(track_df[refund_cols].sum().sum()) if refund_cols else 0.0
    hit_races = int(track_df[hit_cols].fillna(0).astype(int).gt(0).any(axis=1).sum()) if hit_cols else 0
    return {
        "keys": strategy_keys,
        "races": total_races,
        "hits": hit_races,
        "hit_rate": _calc_hit_rate(hit_races, total_races),
        "bet": total_bet,
        "refund": total_refund,
        "profit": total_refund - total_bet,
        "roi": (total_refund / total_bet) if total_bet > 0 else 0.0,
        "holes_per_race": holes,
    }


def _find_best_roi_combo_for_track(track_df: pd.DataFrame, strategy_columns: dict, combo_size: int = 4) -> dict:
    candidate_keys = []
    for key, meta in strategy_columns.items():
        bet_col = meta.get("bet")
        refund_col = meta.get("refund")
        hit_col = meta.get("hit")
        if bet_col not in track_df.columns or refund_col not in track_df.columns or hit_col not in track_df.columns:
            continue
    ...
    best = {}
    for combo in combinations(candidate_keys, combo_size):
        summary = _summarize_strategy_combo(track_df, list(combo), strategy_columns)
        if not best:
            best = summary
            continue
        if (
            summary["roi"] > best["roi"]
            or (
                abs(summary["roi"] - best["roi"]) < 1e-12
                and summary["hit_rate"] > best["hit_rate"]
            )
            or (
                abs(summary["roi"] - best["roi"]) < 1e-12
                and abs(summary["hit_rate"] - best["hit_rate"]) < 1e-12
                and summary["profit"] > best["profit"]
            )
        ):
            best = summary
    return best
...
def print_primary_style_recommendations(from_date: str, to_date: str) -> None:
    admin_profit_groups, admin_profit_strategy_result_columns, admin_profit_strategy_labels = _build_admin_profit_runtime_config()
    race_df = _load_augmented_admin_profit_race_df(from_date, to_date)
    if race_df is None or race_df.empty:
        return
    ...
    for track_name in ["서울", "부산"]:
        ...
        if track_df.empty:
            continue
        stable_keys = list((admin_profit_groups.get(track_name, {}) or {}).get("주력베팅", []))
        stable = _summarize_strategy_combo(track_df, stable_keys, admin_profit_strategy_result_columns)
        attack = _find_best_roi_combo_for_track(track_df, admin_profit_strategy_result_columns, combo_size=4)

        stable_labels = " + ".join(admin_profit_strategy_labels.get(key, key) for key in stable_keys)
        attack_labels = " + ".join(admin_profit_strategy_labels.get(key, key) for key in attack.get("keys", []))

        print(
            f"[{track_name} 안정형]  경주수: {stable.get('races', 0)}  적중경주수: {stable.get('hits', 0)}  "
            f"적중율: {float(stable.get('hit_rate', 0.0) or 0.0):.3f}  총베팅액: {float(stable.get('bet', 0.0) or 0.0):,.0f}원  "
            f"총환수액: {float(stable.get('refund', 0.0) or 0.0):,.0f}원  이익금액: {float(stable.get('profit', 0.0) or 0.0):,.0f}원  "
            f"환수율: {float(stable.get('roi', 0.0) or 0.0):.3f}  구멍수: {int(stable.get('holes_per_race', 0) or 0)}"
        )
        print(f"  전략: {stable_labels}")
        if attack:
            print(
                f"[{track_name} 공격형]  경주수: {attack.get('races', 0)}  적중경주수: {attack.get('hits', 0)}  "
                f"적중율: {float(attack.get('hit_rate', 0.0) or 0.0):.3f}  총베팅액: {float(attack.get('bet', 0.0) or 0.0):,.0f}원  "
                f"총환수액: {float(attack.get('refund', 0.0) or 0.0):,.0f}원  이익금액: {float(attack.get('profit', 0.0) or 0.0):,.0f}원  "
                f"환수율: {float(attack.get('roi', 0.0) or 0.0):.3f}  구멍수: {int(attack.get('holes_per_race', 0) or 0)}"
            )
            print(f"  전략: {attack_labels}")
...
def main() -> None:
    ...
    print_saturday_primary_summary(DEFAULT_FROM_DATE, DEFAULT_TO_DATE)
    print_primary_style_recommendations(DEFAULT_FROM_DATE, DEFAULT_TO_DATE)
