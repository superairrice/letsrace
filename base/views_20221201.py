...
from base.mysqls import (get_award, get_award_race, get_judged, get_paternal,
                         get_paternal_dist, get_pedigree, get_race,
...
def home(request):
    ...
    # weeks = get_weeks(i_rdate, i_awardee='trainer')
    # race = get_race(i_rdate, i_awardee='trainer')
...
def raceResult(request, rcity, rdate, rno, hname, awardee):
    ...
    context = {'records': records, 'r_condition': r_condition, 'hr_records': hr_records,
               'compare_r': compare_r, 'hname': hname, 'judged': judged}
...
def awardStatusTrainer(request):
    ...
    if q == '':
        i_rdate = Racing.objects.values('rdate').distinct()[0]['rdate']
    else:
        i_rdate = q

    weeks = get_last2weeks(i_rdate, i_awardee='trainer')

    status = get_status_training(i_rdate)

    if status:
        pass
    else:
        return render(request, 'base/award_status_trainer.html')

    context = {'weeks': weeks, 'status': status}
