...
def set_common2(rcity, rdate, distance, horse, i_jockey):
    ...
    try:
        ...
        strSql = (
            ...
            + str(distance)
            + """
            and r_flag = '0' and race_speed not in ( '①', '⑨', '⑩' ) 
...
def set_record2(rcity, rdate, distance, horse, i_jockey, weight, i_avg, i_fast, i_slow):
    ...
    for index, r in enumerate(recent_race):
        ...
        # print(index, i_100m, horse, enumerate(recent_race))
    ...
    #     (max + min) / 2,
    #     tval,
    #     max - i_recent5 / i_cnt + 60,
    #     min - i_recent5 / i_cnt - 60,
    # )
    if i_cnt > 0:
...
def set_rank2(rcity, rdate, rno):
    ...
    try:
        ...
        strSql = (
            """ 
            UPDATE exp011s2  a
            SET complex5 = ( select complex from exp011s2 where rcity = a.rcity and rdate = a.rdate and rno = a.rno and rank  = 4 ),
                gap = a.i_complex - ( select i_complex from exp011s2 where rcity = a.rcity and rdate = a.rdate AND rno = a.rno AND rank = a.rank - 1 ),
                gap_back = ( select i_complex from exp011s2 where rcity = a.rcity and rdate = a.rdate AND rno = a.rno AND rank = a.rank + 1 ) - a.i_complex 
            WHERE rcity = '"""
            + rcity
            + """'
                    AND rdate = '"""
