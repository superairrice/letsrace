...
def compute_darkhorse_score(h: Dict[str, Any], avg: Dict[str, float]) -> float:
    """
    darkhorse_score (m_rank ≥ 4 전용 부스터 적용 버전)

    - 기본 편차 점수는 항상 계산
    - 기록 낮음 부스터는 m_rank ≥ 4 인 경우에만,
      그리고 초/종반+트렌드 우위가 확실할 때만 발동
    """
    ...
    s1f_diff = s1f - float(avg.get("초반200") or 0.0)
    g3f_diff = g3f - float(avg.get("종반600") or 0.0)
    trend_diff = trend - float(avg.get("최근8") or 0.0)
    rec_diff = rec - float(avg.get("기록점수") or 0.0)

    # 1) 기본 편차 점수 (완만한 스케일)
    score = 0.04 * s1f_diff + 0.06 * g3f_diff + 0.05 * trend_diff

    # 2) 기록 낮음 부스터 (m_rank ≥ 4 전용)
    rec_penalty_boost = 0.0
    m_rank = int(h.get("m_rank") or 99)

    if m_rank >= 4 and rec_diff <= -8:
        # 조건 A/B 중 하나 충족 시만 발동
        cond_A = (g3f_diff >= 10) and (trend_diff >= 6)  # 종반형 복병
        cond_B = (s1f_diff >= 10) and (trend_diff >= 6)  # 선행형 복병

        if cond_A or cond_B:
            # 기록 열세 깊이에 따른 연속 스케일 (0~2.0)
            depth = min(20.0, max(8.0, -rec_diff))  # 8~20
            rec_penalty_boost = (depth - 8.0) / 6.0
            rec_penalty_boost = max(0.0, min(2.0, rec_penalty_boost))
            score += rec_penalty_boost

    # 3) 기록이 오히려 좋은 말은 복병성 억제
    if rec_diff >= 6:
        score -= 0.6

    # 4) 디버그용 저장
    h["dark_s1f_diff"] = s1f_diff
    h["dark_g3f_diff"] = g3f_diff
    h["dark_trend_diff"] = trend_diff
    h["dark_rec_diff"] = rec_diff
    h["dark_rec_boost"] = rec_penalty_boost

    # 5) 클램프
    dh = max(-3.0, min(12.0, round(score, 2)))
    ...
    if not horses:
        ...
        try:
            ...
            def _dark_comment(row):
                key = (
                    (row.get("gate"), row.get("horse"))
                    if isinstance(row, dict)
                    else (row["gate"], row["horse"])
                )
                h = by_key.get(key)
                if not h:
                    # fallback: darkhorse_score만으로 간단 표기
                    ds = (
                        float(row.get("darkhorse_score", 0.0))
                        if isinstance(row, dict)
                        else (
                            float(row["darkhorse_score"])
                            if "darkhorse_score" in row
                            else 0.0
                        )
                    )
        ...
        def _build_overview(horses_list, df_table):
            ...
            # 전체 빠른 초반/강한 종반 개수
            fast_early = sum(
                1 for h in horses_list if (h.get("dark_s1f_diff") or 0) >= 8
            )
            strong_closer = sum(
                1 for h in horses_list if (h.get("dark_g3f_diff") or 0) >= 15
            )
            ...
            if top_h:
                ...
                if ts < 60:
                    risks.append(
                        f"최상위 {top_h[0].get('horse')} 신뢰도 {float(ts):.1f}로 낮음"
                    )
            if len(top_h) > 1:
                ts2 = top_h[1].get("trust_score") or top_h[1].get("trust") or 0.0
                if ts2 < 60:
                    risks.append(
                        f"상위권 {top_h[1].get('horse')} 신뢰도 {float(ts2):.1f}로 낮음"
                    )
            ...
            for _, r in df_table.iterrows():
                ...
                if not h:
                    continue
                dark_score = float(
                    h.get("darkhorse_score") or r.get("darkhorse_score") or 0.0
                )
            ...
            if selected:
                ...
                for h in selected:
                    ...
                    dark_lines.append(f"    ⤷ 선정 코멘트: {'; '.join(sel_comment)}")
        ...
        # =========================
        # 모듈 제공 함수 (raceCalculation 등에서 import 호출 가능)
        # =========================

        # NOTE: 아래 함수들은 모듈 전역으로 이동하여 재사용 가능하도록 제공합니다.
        # 기존 __main__ 흐름은 샘플 실행만 유지합니다.
# ====================================
# 공개 유틸 함수 (views 등에서 import 사용)
# ====================================


def ensure_exp011_score_columns() -> None:
    """exp011에 점수 관련 컬럼이 없다면 추가 (가능한 환경에서만).

    추가 컬럼:
      - f_rank INT, f_score DECIMAL(8,2)
      - trust_score DECIMAL(6,1), trust_label VARCHAR(20)
      - dh_score DECIMAL(6,2), comment_dh VARCHAR(255)
    """
    alters = [
        "ALTER TABLE The1.exp011 ADD COLUMN IF NOT EXISTS f_rank INT NULL",
        "ALTER TABLE The1.exp011 ADD COLUMN IF NOT EXISTS f_score DECIMAL(8,2) NULL",
        "ALTER TABLE The1.exp011 ADD COLUMN IF NOT EXISTS trust_score DECIMAL(6,1) NULL",
        "ALTER TABLE The1.exp011 ADD COLUMN IF NOT EXISTS trust_label VARCHAR(20) NULL",
        "ALTER TABLE The1.exp011 ADD COLUMN IF NOT EXISTS dh_score DECIMAL(6,2) NULL",
        "ALTER TABLE The1.exp011 ADD COLUMN IF NOT EXISTS comment_dh VARCHAR(255) NULL",
    ]
        ...
        with c.cursor() as cur:
            for sql in alters:
                try:
                    ...
                    # MySQL 5.x 등에서 IF NOT EXISTS 미지원 시 무시
                    pass
        c.commit()


def compute_scores_df_for_race(rcity: str, rdate: str, rno: int) -> pd.DataFrame:
    """한 경주(rcity, rdate, rno)에 대한 점수 DataFrame 생성."""
    horses = load_race(rcity, rdate, rno)
    if not horses:
        return pd.DataFrame()
    return listup_scores(horses)
...
def update_exp011_for_race(
    ...
    if df_scores is None:
        df_scores = compute_scores_df_for_race(rcity, rdate, rno)
    if df_scores is None or df_scores.empty:
        return 0

    df_sorted = df_scores.sort_values("final_score", ascending=False).reset_index(
        drop=True
    )
...
def get_races_in_period(
    ...
    """기간 내 경주 목록 (rcity, rdate, rno) 조회. rcity 필터 선택."""
...
def update_exp011_for_period(
    ...
    """기간(from_date~to_date) 내 모든 경주(exp010 기준)에 대해 exp011 점수 컬럼 업데이트.
    ...
    for rc, rd, rn in races:
        try:
            cnt = update_exp011_for_race(rc, rd, rn, ensure_columns=False)
            if cnt:
                updated += 1
                if verbose:
                    print(f"  - 업데이트 완료: {rc} {rd} R{rn} ({cnt}행)")
