...
def compute_darkhorse_score(
    row: pd.Series, df_scored: pd.DataFrame, pace_info: dict, top6_df: pd.DataFrame
) -> float:
    """
    복병 점수 계산.
    - 상위권 대비 종반600 / 초반200 우위
    - 트렌드 우위
    - 부담중량 이득
    - 전개(페이스 vs 스타일) 보정
    """
    # 상위권(여기선 final_score 기준 상위6) 기준
    ...
    pace_bonus = 0.0
    if "빠른 페이스" in pace_desc and style == "closer":
        pace_bonus = 10.0
    elif ("느린" in pace_desc or "느린~보통" in pace_desc) and style == "front":
        pace_bonus = 7.0

    darkhorse_score = g3f_dom + s1f_dom + trend_dom + weight_score + pace_bonus
    return darkhorse_score


def get_darkhorse_candidates(
    df_scored: pd.DataFrame, pace_info: dict, trust_level: str
):
    """
    복병 후보 리스트 전체 반환 (6복조 외).
    [{'gate', 'horse', 'final_score', 'darkhorse_score'}, ...]
    """
    # final_score 기준 정렬
    df_sorted = df_scored.sort_values("final_score", ascending=False)

    # 복병은 "최종 6복조"에 없어야 하므로, m_rank 기반 6복조 게이트 제외
    main_top6_gates = set(get_6복조_line(df_scored))

    # 상위권 기준(도미넌스 기준용) → final_score 상위6
    ...
    for _, row in df_sorted.iterrows():
        gate = int(row["gate"])
        if gate in main_top6_gates:
            continue  # 6복조 편입마는 복병 후보 제외
...
def get_darkhorse(df_scored: pd.DataFrame, pace_info: dict, trust_level: str):
    """
    6복조 외 복병 1마리 선정.
    - 기본: 정규 후보 중 darkhorse_score 최상위
    - 강축: threshold 높게
    - 약한축: threshold 완화 + 후보 없으면 fallback으로 6복조 외 최상위 final_score 말 선정
    """
    df_sorted = df_scored.sort_values("final_score", ascending=False)
    main_top6_gates = set(get_6복조_line(df_scored))
    top6_df = df_sorted.head(6)
    ...
    # 후보 자체가 없으면: 약한축일 때는 fallback으로 "6복조 외 final_score 최고 말" 선정
    if not candidates_raw:
        if trust_level == "약한축":
            # 6복조에 없는 말들 중 final_score 최상위
            df_ex = df_sorted[~df_sorted["gate"].isin(main_top6_gates)]
            if len(df_ex) == 0:
                return None, None
            fallback_row = df_ex.iloc[0]
...
def print_trust_and_betting(df_scored: pd.DataFrame):
    ...
    # 6복조 라인 (m_rank 기준)
    top6_gates = get_6복조_line(df_scored)
    print("=== 편성 상태 / 6복조 라인 ===")
    ...
    # 복병 선정
    # ------------------------
    darkhorse_row, dh_score = get_darkhorse(df_scored, pace_info, trust["trust_level"])

    print("=== 복병 선정 결과 ===")
    if darkhorse_row is None:
        print("복병 없음")
        print()
    else:
        msg = (
            f"복병: 게이트 {int(darkhorse_row['gate'])} / {darkhorse_row['horse']} "
            f"/ 점수 {dh_score:.2f}"
        )
        if dh_score <= 0:
            msg += "  (약한축 Fallback 복병)"
    ...
    # ------------------------
    print("=== 복병 후보 리스트 ===")
    candidates = get_darkhorse_candidates(df_scored, pace_info, trust["trust_level"])
    if candidates:
        for c in candidates:
            print(
    ...
    # Fallback 복병 상세 표시
    if darkhorse_row is not None and dh_score == 0:
        print("※ Fallback 복병 (정규 후보 조건 미통과):")
        print(
            f"게이트 {int(darkhorse_row['gate'])} / {darkhorse_row['horse']} / "
            f"final_score={float(darkhorse_row['final_score']):.2f} / "
            f"style={float(darkhorse_row['style_new']):.2f}"
        )
        print()
    ...
    print(f"=== 삼복승식 베팅 플랜 (총 10000원 + 복병라인 3000원 가정) ===")
    print(
    ...
    if mode == "강축":
        ...
        print("  → 메인 비중: 100% / 복병 라인: 소액 가미 (약한축 보조 시나리오)")
    print()

    # ------------------------
    # 메인 삼복조 티켓
    # ------------------------
    tickets_main = make_trifecta_tickets(
        top6_gates, anchor_gate, trust["trust_level"], total_budget=10000
    )

    print("▶ 메인 삼복조 티켓")
    for t in tickets_main:
        g1, g2, g3 = t["gates"]
        amount = t["amount"]
        h1 = df_scored.loc[df_scored["gate"] == g1, "horse"].iloc[0]
        h2 = df_scored.loc[df_scored["gate"] == g2, "horse"].iloc[0]
        h3 = df_scored.loc[df_scored["gate"] == g3, "horse"].iloc[0]
    ...
    # ------------------------
    print("▶ 복병 라인 삼복조 티켓")
    if darkhorse_row is not None:
        dark_gate = int(darkhorse_row["gate"])
        tickets_dark = make_darkhorse_tickets(
            top6_gates, anchor_gate, dark_gate, total_budget=3000
        )
        if not tickets_dark:
            print("  생성된 복병 조합 없음")
        else:
            for t in tickets_dark:
                g1, g2, g3 = t["gates"]
                amount = t["amount"]
                h1 = df_scored.loc[df_scored["gate"] == g1, "horse"].iloc[0]
                h2 = df_scored.loc[df_scored["gate"] == g2, "horse"].iloc[0]
