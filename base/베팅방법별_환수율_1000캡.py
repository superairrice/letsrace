...
def build_admin_profit_analysis_payload_for_range(from_date: str, to_date: str) -> dict:
    ...
    common_primary_keys = []
    for track_name in ["서울", "부산"]:
        for key in admin_profit_groups.get(track_name, {}).get("주력베팅", []):
            if key not in common_primary_keys:
                common_primary_keys.append(key)
    for track_name in ["서울", "부산"]:
        admin_profit_groups.setdefault(track_name, {})["주력베팅"] = list(common_primary_keys)
    ...
    if race_df is not None and hasattr(race_df, "columns") and not race_df.empty:
        ...
        trio_odds = pd.to_numeric(race_df.get("삼복승식배당율"), errors="coerce").clip(upper=MAX_DISPLAY_ODDS)
        trifecta_odds = pd.to_numeric(race_df.get("삼쌍승식배당율"), errors="coerce").clip(upper=MAX_DISPLAY_ODDS)
        ...
        for key, column_meta in admin_profit_strategy_result_columns.items():
            ...
            if "삼복" in label:
                ...
                continue
            hit_mask = pd.to_numeric(race_df[hit_col], errors="coerce").fillna(0).gt(0).astype(int)
            refund_values = (odds.fillna(0.0) * 100 * hit_mask).astype(float)
