...
MAX_HOLES = 30
DEFAULT_OUTPUT_PATH = (
    Path("/Users/Super007/Documents") / "r_pop1_8_삼쌍_30구멍이하_전수탐색.csv"
)
...
def _strategy_label(first: tuple[int, ...], second: tuple[int, ...], third: tuple[int, ...]) -> str:
    return (
        f"1착({_format_rank_group(first)}) / "
        f"2착({_format_rank_group(second)}) / "
        f"3착({_format_rank_group(third)}) 삼쌍"
    )
        ...
        race_rows.append(
            {
                ...
                "odds": float(g["삼쌍승식배당율"].iloc[0] or 0.0),
            }
        )
    return race_rows
...
def _evaluate_strategies(race_rows: list[dict], strategies: list[dict], bet_unit: int) -> pd.DataFrame:
    ...
    for strategy in strategies:
        ...
        for race in race_rows:
            ...
            a1, a2, a3 = race["actual_top3"]
            if a1 in first_set and a2 in second_set and a3 in third_set:
                hits += 1
        ...
        results.append(
            {
                "전략명": strategy["label"],
                "1착군": _format_rank_group(first),
                "2착군": _format_rank_group(second),
                "3착군": _format_rank_group(third),
                "구멍수": holes,
                "적용경주수": valid_races,
                "전체기준경주수": total_races,
                "적중수": hits,
...
def main():
    ...
    print(
        f"[r_pop 1~8 삼쌍 전수탐색] 기간 {from_date}~{to_date}  "
