...
DEFAULT_OUTPUT_PATH = "/Users/Super007/Documents/r_pop_total_new_서울_주력_토요일.csv"
...
def build_saturday_primary_summary(race_df: pd.DataFrame) -> pd.DataFrame:
    """서울 주력베팅을 토요일 기준으로 묶어 적중/환수 현황을 만든다."""
    if race_df.empty:
        return pd.DataFrame()
...
def print_saturday_primary_summary(weekly_df: pd.DataFrame) -> None:
    ...
    total_races = int(weekly_df["경주수"].sum())
    total_hits = int(weekly_df["적중경주수"].sum())
    total_bet = float(weekly_df["총베팅액"].sum())
    total_refund = float(weekly_df["총환수액"].sum())
    total_profit = float(weekly_df["이익금액"].sum())
    ...
    print(
        f"[총계]  경주수: {total_races}  "
        f"적중경주수: {total_hits}  적중율: {total_hit_rate:.3f}  "
        f"총베팅액: {int(total_bet):,}원  총환수액: {total_refund:,.1f}원  "
        f"이익금액: {total_profit:,.1f}원  환수율: {total_refund_rate:.3f}"
    )


def save_weekly_df(weekly_df: pd.DataFrame, output_path: str = DEFAULT_OUTPUT_PATH) -> None:
    """토요일 기준 서울 주력베팅 현황 CSV 저장."""
    if weekly_df.empty:
        return
